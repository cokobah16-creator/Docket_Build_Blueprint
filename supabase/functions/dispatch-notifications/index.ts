...
function render(n: Row, firm: string, tz: string): { subject: string; text: string; url: string } {
  ...
  switch (n.event) {
    ...
    case 'document_signed':          return { subject: `Docket: ${p.name ?? 'a document'} signed`, text: `${p.signer ?? 'The signer'} signed ${p.name ?? 'a document'}. The document is now locked on the signed version.`, url: `/firm/matters/${p.matter_id}?tab=documents` };
