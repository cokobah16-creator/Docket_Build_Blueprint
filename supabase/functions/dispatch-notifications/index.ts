...
function render(n: Row, firm: string, tz: string): { subject: string; text: string; url: string } {
  ...
  switch (n.event) {
    ...
    case 'registry_notice_received': return { subject: `Docket: the ${p.court_name ?? 'court'} registry has listed ${p.suit_number ?? 'a suit'}`, text: `The ${p.court_name ?? 'court'} registry has listed ${p.suit_number ?? 'a suit'}${p.purpose ? ` for ${p.purpose}` : ''} on ${p.listed_on ? fmtDay(String(p.listed_on)) : 'a date'}. Nothing is in your diary until you confirm it: open Sittings to confirm the date or say it is not yours.`, url: `/firm/sittings#registry` };
    case 'registry_notice_withdrawn': return { subject: `Docket: the registry has withdrawn a listing you confirmed`, text: `The ${p.court_name ?? 'court'} registry has withdrawn its notice listing ${p.suit_number ?? 'a suit'} on ${p.listed_on ? fmtDay(String(p.listed_on)) : 'a date'}${p.reason ? ` — "${p.reason}"` : ''}. The date is still in your diary; check with the registry before relying on it, and vacate it yourself if it no longer stands.`, url: `/firm/sittings` };
