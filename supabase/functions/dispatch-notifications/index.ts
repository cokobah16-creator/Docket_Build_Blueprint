...
function render(n: Row, firm: string, tz: string): { subject: string; text: string; url: string } {
  ...
  switch (n.event) {
    ...
    case 'process_served':           return { subject: `Docket: ${p.process_title} served on your firm`, text: `${p.serving_firm_name} served ${p.process_title} in ${p.case_title ?? 'a matter'}${p.suit_number ? ` (${p.suit_number})` : ''}. Open your service inbox to read and acknowledge it.`, url: `/firm/service/${p.process_service_id}` };
    case 'service_acknowledged':     return { subject: `Docket: service of ${p.process_title} acknowledged`, text: `${p.by_firm} acknowledged service of ${p.process_title}.`, url: p.matter_id ? `/firm/matters/${p.matter_id}` : `/firm/service/${p.process_service_id}` };
