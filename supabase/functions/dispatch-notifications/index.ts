...
function render(n: Row, firm: string, tz: string): { subject: string; text: string; url: string } {
  ...
  switch (n.event) {
    ...
    case 'document_requested':       return { subject: `${firm}: a document is needed`, text: `${firm} has asked you for: ${p.title}${p.due_on ? `, by ${fmtDay(String(p.due_on))}` : ''}. Upload it in the app.`, url: `/app/matters/${p.matter_id}?tab=documents` };
