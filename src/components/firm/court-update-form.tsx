...
export function CourtUpdateForm({
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <fieldset>
        <legend className="text-sm font-medium text-ink">What happened in court?</legend>
      ...
      <div className="flex flex-wrap items-end gap-3">
        <div className="min-w-[10rem] flex-1">
          <label htmlFor={`cu_sat_${uid}`} className="text-sm font-medium text-ink">Date of the sitting</label>
          <input
            id={`cu_sat_${uid}`} type="date" value={satOn} max={today}
            onChange={(e) => setSatOn(e.target.value)} className={field}
          />
        </div>
        <p className="pb-2 text-xs text-ink-muted">Times in {timezone}</p>
      </div>

      {outcome === "adjourned" && (
        <fieldset>
          <legend className="text-sm font-medium text-ink">At whose instance?</legend>
      ...
      <div className="space-y-2 rounded-lg border border-hairline p-3">
        <div className="flex flex-wrap items-end gap-3">
          <div className="min-w-[9rem] flex-1">
            <label htmlFor={`cu_next_${uid}`} className="text-sm font-medium text-ink">
          ...
          <div className="w-28">
            <label htmlFor={`cu_time_${uid}`} className="text-sm font-medium text-ink">Time</label>
            <input
        ...
        {checking && <p className="text-xs text-ink-muted">Checking the court calendar…</p>}
        ...
        {nextDate && (
          <>
            <p className="text-sm font-medium text-ink">Fixed for</p>
      ...
      <div>
        <label htmlFor={`cu_note_${uid}`} className="text-sm font-medium text-ink">Note to the client</label>
        <p className="text-xs text-ink-muted">Plain language. This is what your client reads in their app.</p>
        <textarea
      ...
      <div>
        ...
        {!showMore && currentCourtName && (
          <p className="text-xs text-ink-muted">Filed against {currentCourtName}{judicialDivision ? `, ${judicialDivision}` : ""}.</p>
        )}
        {showMore && (
          <div className="mt-3 space-y-3 rounded-lg border border-hairline p-3">
            ...
            />
            <div>
              <label htmlFor={`cu_division_${uid}`} className="text-sm font-medium text-ink">Judicial division</label>
              <input
            ...
            <div>
              <label htmlFor={`cu_courtname_${uid}`} className="text-sm font-medium text-ink">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                <label htmlFor={`cu_judge_${uid}`} className="text-sm font-medium text-ink">Judge</label>
                <input
              ...
              <div>
                <label htmlFor={`cu_room_${uid}`} className="text-sm font-medium text-ink">Courtroom</label>
                <input
      ...
      <div className="space-y-2">
        ...
        {!outcome && <p className="text-center text-xs text-ink-muted">Tap what happened in court to begin.</p>}
        {outcome && (
