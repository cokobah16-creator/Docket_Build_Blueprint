...
export function RegistryNoticesCard({ firmId, firmParam, timezone, notices }: {
  ...
  return (
    <Card className={open.length > 0 ? "border-[#141414]" : undefined}>
      ...
      <CardBody className="border-b border-gray-100 text-[12.5px] leading-[1.55] text-[#57534E]">
      ...
      {open.length === 0 ? (
        <CardBody className="text-sm text-gray-600">Nothing waiting. A new listing appears here the moment the registry publishes it.</CardBody>
      ) : (
            ...
            return (
              <li key={k} className="px-4 py-4 sm:px-5">
                ...
                {n.listed_on_non_sitting_day && (
                  <p className="mt-1 text-xs font-medium text-[#B42318]">This day is a weekend or a public holiday. The diary will refuse it; ask the registry.</p>
                )}
                {n.listed_in_vacation && !n.listed_on_non_sitting_day && (
                  <p className="mt-1 text-xs font-medium text-amber-800">This day falls inside the court&rsquo;s vacation — a vacation judge&rsquo;s sitting. It will be diarised and marked as such.</p>
                )}
                ...
                <div className="mt-2 flex flex-wrap items-end gap-3">
                  {!sameDayAlready && (
                    <label className="block text-xs text-gray-700">Time{n.listed_time ? "" : " (the registry gave none)"}
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  ...
                  <Link href={`/firm/matters/${n.matter_id}${firmParam ? `?firm=${encodeURIComponent(firmParam)}` : ""}`} className="self-center text-xs text-brand underline">Open the matter →</Link>
                </div>
              </li>
            );
          })}
      ...
      {decided.length > 0 && (
        <details className="border-t border-gray-100 px-4 py-3 sm:px-5">
          <summary className="cursor-pointer text-xs font-medium text-gray-700">Decided ({decided.length})</summary>
          <ul className="mt-2 space-y-1.5">
            {decided.map((n) => (
              <li key={key(n)} className="text-xs text-gray-600">
