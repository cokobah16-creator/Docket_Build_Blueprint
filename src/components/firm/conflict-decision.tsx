      ...
      {matches.map((m, i) => (
        <li key={`${m.kind}-${m.name}-${m.matter_id ?? "r"}-${i}`} className="px-3 py-2 text-sm">
          <p className="font-medium text-ink">
            {m.name}
            <span className="ml-2 text-xs font-normal text-ink-muted">{STRENGTH_LABELS[m.strength]} match for &ldquo;{m.searched}&rdquo;</span>
          </p>
...
export function DecideCheck({
  ...
  return (
    <form onSubmit={submit} className="mt-2 space-y-2 rounded-lg border border-hairline p-3">
      ...
      <p className="text-sm font-medium text-ink">Your decision</p>
          ...
          <button
            ...
            className={
              outcome === o
                ...
                : "flex min-h-[44px] items-center rounded-full border border-edge bg-raised px-4 text-sm text-ink hover:border-brand"
            }
          >
            {OUTCOME_LABELS[o]}
          </button>
      ...
      <div>
        <label htmlFor={`note-${checkId}`} className="text-sm font-medium text-ink">
