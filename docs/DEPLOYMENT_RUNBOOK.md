...
| **Edge Functions** | `paystack-webhook` **v7** · `dispatch-notifications` **v10** (renders every event through Wave 4f, `registry_notice_received` / `registry_notice_withdrawn` included) · `video-session` **v2** · `storage-manifest` **v1** · `calendar-feed` **v1** · `delivery-receipts` **v1** · `extract-text` **v2** · `partner-api` **v1** · `partner-webhooks` **v1**. Nine functions, eight `--no-verify-jwt`; `video-session` is the only one that keeps JWT verification on. **v6/v2 were the 14 Sep 2026 reconciliation, and v7/v10 the missing-secret guards of the same day** (§3a): comment-only redeploys from the committed files, after which every production function'''s `ezbr_sha256` matches staging'''s except `partner-api`, which differs by design | MCP `list_edge_functions`; `cron.job`; `cron.job_run_details` |
