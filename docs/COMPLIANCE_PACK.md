...
| **Cloudflare R2** | a byte-for-byte copy of every object in the `documents` and `intake-uploads` buckets — so **the documents themselves**, including whatever a client uploaded. Region `enam`, the same jurisdiction as the Supabase primary, so this adds a processor and no new country. Key names are `<bucket>/<path>`, which carry firm and document ids and no client name | `supabase/functions/storage-replicate` (migration 50) |
