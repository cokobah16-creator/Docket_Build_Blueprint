...
| **PostHog** | **Nothing unless the visitor chose "Allow analytics"** in the cookie banner. The choice is kept in the `docket_consent` cookie, per browser and per site address, and the anonymous visitor cookie `docket_did` exists only after it. Then: a tenant site viewed (`firm_id`, `firm_slug`), a booking started (`firm_id`, `firm_slug`, `service_id`, `amount_minor`, `currency`), a firm registered (`firm_id`, `state_code`), and at sign-in an `$identify` that links `docket_did` to the account's uuid. Every event also carries `$lib` and `environment`. A matter opened is named in the code but not sent: the lawyer's browser opens it, so the client's own choice cannot be read. A payment and attendance are named and emitted by nothing: a payment is only trustworthy from the Paystack webhook, which runs in Deno and does not share this module, and attendance has no single moment that makes it true. The distinct id is the account's uuid or `docket_did`. **Nothing identifies a person by name, phone or email.** Default host `https://eu.i.posthog.com`. Inert with no `POSTHOG_KEY` | `src/lib/observability/posthog.ts`, gated by `src/lib/observability/consent.ts` and `middleware.ts` |
