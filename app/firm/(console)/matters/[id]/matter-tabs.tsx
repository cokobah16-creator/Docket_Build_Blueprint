...
import { appButtonClass } from "@/components/app/button";
...
export function MatterTabs({
  ...
  return (
    <nav aria-label="Matter sections" className="-mx-4 overflow-x-auto px-4 sm:mx-0 sm:px-0">
      <ul className="flex gap-[7px] pb-1">
        {tabs.map((t) => {
          ...
          return (
            <li key={t.key}>
              <Link
                ...
                className={cn(
                  "flex min-h-[44px] shrink-0 items-center whitespace-nowrap rounded-full border px-3.5 text-[12.5px] font-medium",
                  current
    ...
    <span className={cn("inline-flex flex-wrap items-center gap-2", className)}>
      ...
      {state === "manual" && (
        <input
          ...
          className="w-full rounded-[9px] border border-dk-field bg-white px-3 py-2 font-mono text-[12px] text-dk-body sm:w-80"
        />
      )}
    </span>
  );
