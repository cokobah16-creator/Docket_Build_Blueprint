    ...
    <div className="divide-y divide-hairline">
      ...
      <section className="px-4 py-4 sm:px-5">
        ...
        <form onSubmit={add} className="mt-3 space-y-3">
          <div>
            <label htmlFor="task-title" className="text-sm font-medium text-ink">What has to be done <span className="text-red-700">*</span></label>
            <input
              id="task-title"
              value={title}
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <div>
              <label htmlFor="task-due-day" className="text-sm font-medium text-ink">Due</label>
              <input id="task-due-day" type="date" value={dueDay} onChange={(e) => setDueDay(e.target.value)} className={field} />
            </div>
            <div>
              <label htmlFor="task-due-time" className="text-sm font-medium text-ink">By</label>
              <input id="task-due-time" type="time" value={dueTime} onChange={(e) => setDueTime(e.target.value)} disabled={!dueDay} className={field} />
            </div>
            <div>
              <label htmlFor="task-assignee" className="text-sm font-medium text-ink">Who</label>
          ...
          <p className="text-xs text-ink-muted">Times are in {timezone}.</p>
      ...
      <section className="px-4 py-4 sm:px-5">
        <h3 className="font-heading text-base font-semibold text-ink">
          Open {open.length > 0 && <span className="text-sm font-normal text-ink-muted">({open.length}{overdueCount > 0 ? `, ${overdueCount} overdue` : ""})</span>}
        </h3>
        {open.length === 0 ? (
              ...
              return (
                <li key={t.id} className="flex flex-wrap items-start justify-between gap-3 py-3">
                  <div className="min-w-0">
                    <p className={cn("text-sm font-medium", overdue ? "text-red-800" : "text-ink")}>{t.title}</p>
                    <p className={cn("text-xs", overdue ? "font-medium text-red-700" : "text-ink-muted")}>
                      {t.due_at
            ...
            <ul className="mt-3 divide-y divide-hairline">
              {done.map((t) => (
                <li key={t.id} className="py-3">
                  <p className="text-sm text-ink-muted line-through">{t.title}</p>
