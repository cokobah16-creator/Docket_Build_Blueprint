...
export function Figures({ m, currency = "NGN" }: { m: FirmMetrics; currency?: string }) {
  return (
    <div className="space-y-4">
      <section>
        <h3 className="text-sm font-semibold text-ink">Consultations</h3>
      ...
      <section>
        <h3 className="text-sm font-semibold text-ink">From a consultation to a matter</h3>
        <div className="mt-2 grid gap-2 sm:grid-cols-3">
        ...
        <p className="mt-1 text-xs text-ink-muted">An inference, not a record: nothing joins a consultation to a matter.</p>
      </section>
      <section>
        <h3 className="text-sm font-semibold text-ink">Telling the client</h3>
        <div className="mt-2 grid gap-2 sm:grid-cols-3">
      ...
      <section>
        <h3 className="text-sm font-semibold text-ink">Answering</h3>
        <div className="mt-2 grid gap-2 sm:grid-cols-3">
      ...
      <section>
        <h3 className="text-sm font-semibold text-ink">Money and work</h3>
        <div className="mt-2 grid gap-2 sm:grid-cols-2">
      ...
      <section>
        <h3 className="text-sm font-semibold text-ink">Clients</h3>
        <div className="mt-2 grid gap-2 sm:grid-cols-2">
      ...
      {m.caveats.length > 0 && (
        <section className="rounded-lg bg-sunken p-3">
          <h3 className="text-sm font-semibold text-ink">What these numbers do not say</h3>
          <ul className="mt-1 list-disc space-y-1 pl-5 text-xs text-ink">
            ...
            {[7, 30, 90, 180].map((d) => (
              <button key={d} type="button" onClick={() => move(d)}
                className={d === windowDays
                  ...
                  : "min-h-[36px] rounded-full border border-edge bg-raised px-3 text-sm text-ink hover:border-brand"}>
                Last {d} days
              </button>
            ))}
          </div>
          {metrics ? <Figures m={metrics} />
            : canWrite ? <p className="text-sm text-ink-muted">No figures — the read did not return.</p>
            : <p className="text-sm text-ink-muted">The figures for a window are an owner&apos;s or administrator&apos;s to read.</p>}
        </CardBody>
      </Card>
          ...
          <CardBody>
            ...
            <form onSubmit={record} className="space-y-3">
              ...
              <fieldset className="rounded-lg border border-hairline p-3">
                <legend className="px-1 text-sm font-medium text-ink">Before Docket — what the firm says</legend>
                <p className="text-xs text-ink-muted">Optional, and every one of them is recorded as a statement by you on today&apos;s date, never as a figure Docket computed.</p>
                ...
                <li key={b.id} className="py-3">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-ink-muted">Taken {fmtFull.format(new Date(b.taken_at))}{b.note ? ` · ${b.note}` : ""}</p>
                    </div>
