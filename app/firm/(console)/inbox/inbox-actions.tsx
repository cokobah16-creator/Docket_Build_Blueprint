...
export function FileServiceForm({
  ...
  if (!open) {
    return (
      <div className="space-y-1">
        ...
        {!filedMatterId && (
          <p className="text-xs text-ink-muted">Filing puts it on your own matter&apos;s timeline, for your firm only.</p>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3 rounded-lg border border-hairline bg-sunken p-3">
      ...
      <div>
        <label htmlFor={`file_matter_${serviceId}`} className="text-sm font-medium text-ink">
      ...
      <div>
        <label htmlFor={`file_due_${serviceId}`} className="text-sm font-medium text-ink">Response falls due</label>
        <input
      ...
      <div>
        <label htmlFor={`file_note_${serviceId}`} className="text-sm font-medium text-ink">Note for the file</label>
        <textarea
        ...
        />
        <p className="mt-1 text-xs text-ink-muted">Filed as an internal entry on your matter. The firm that served it never sees this.</p>
      </div>
      <div className="flex flex-wrap gap-2">
...
export function OpenProcessButton({
  ...
  if (!versionId) {
    return <p className="text-xs text-ink-muted">No file was attached to this service record.</p>;
  }
    ...
    <div className="space-y-1">
      ...
      {url && (
        <p className="text-sm">
          ...
          <span className="text-xs text-ink-muted">— the link lasts two minutes.</span>
        </p>
      )}
