...
/** A word for each kind of update, shown as a small label — never a pictograph. */
const KIND_LABEL: Record<string, string> = {
  court_sitting: "Court", consultation: "Consult", appointment: "Meeting", filing: "Filing", correspondence: "Letter",
  milestone: "Milestone", fee: "Fees", document: "Document", note: "Note", status_change: "Status",
};
...
export default async function FirmOverviewPage({ searchParams }: { searchParams: Promise<{ firm?: string }> }) {
  ...
  const tiles = overview
    ? [
        { label: "Open matters", value: String(overview.open_matters), href: "/firm/matters?open=1" },
        { label: "Sittings to chase", value: String(overview.sittings_due), href: "/firm/sittings" },
        { label: "Court dates, 30 days", value: String(overview.court_dates_30d), href: "/firm/sittings" },
        { label: "Upcoming consultations", value: String(overview.upcoming_appointments), href: "/firm/appointments?view=upcoming" },
        { label: "Unread client messages", value: String(overview.unread_messages), href: "/firm/messages" },
        { label: "Overdue tasks", value: String(overview.overdue_tasks), href: "/firm/tasks?view=overdue" },
        { label: "Client uploads to review", value: String(overview.client_uploads), href: "/firm/uploads" },
    ...
    <div className="space-y-5">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <h1 className="font-heading text-21 font-bold tracking-[-0.02em] text-[#141414]">Reports</h1>
      ...
      {overview ? (
        <section aria-label="Firm totals" className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          {tiles.map((t) => (
            <Link key={t.label} href={t.href} className="rounded-card border border-hairline bg-raised p-3 hover:border-edge">
              ...
              {days.map((d, index) => {
                ...
                return (
                  <div
                    key={d.ymd}
                    className={cn(
                      "rounded-card border bg-raised p-3",
                      index === 0 ? "border-brand" : "border-hairline",
                ...
                {visibleFeed.map((u) => {
                  ...
                  return (
                    <li key={u.id} className="flex gap-3 px-5 py-4">
                      <span className="mt-0.5 w-[72px] shrink-0 text-11 font-semibold uppercase tracking-[0.04em] text-ink-muted">{KIND_LABEL[u.kind] ?? "Update"}</span>
              ...
              <Table>
                ...
                <TBody>
                  ...
                  {(unattributedOriginated > 0 || unattributedHandling > 0) && (
                    <TR>
                      <TD className="text-ink-muted">Not attributed</TD>
                      <TD className="text-right tabular-nums text-ink-muted">{unattributedOriginated}</TD>
                      <TD className="text-right tabular-nums text-ink-muted">{unattributedHandling}</TD>
                      {feesByLawyer && <TD className="text-right text-ink-muted">—</TD>}
                    </TR>
                  )}
                  <TR>
                    <TD className="font-medium text-ink">Total matters</TD>
                    <TD className="text-right font-medium tabular-nums text-ink">{matters.length}</TD>
                    <TD className="text-right font-medium tabular-nums text-ink">{matters.length}</TD>
