...
const quietButton = "min-h-[44px] rounded-lg border border-edge px-4 py-2.5 text-sm font-medium text-ink hover:bg-hover disabled:opacity-50";
              ...
              <li key={r.id} className="py-4">
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  <p className="text-sm font-semibold text-ink">
                    ...
                    {r.retired_on ? <span className="ml-2 rounded-full bg-sunken px-2 py-0.5 text-xs text-ink">retired {r.retired_on}</span> : null}
                  </p>
                  <p className="text-xs text-ink-muted">{r.level ? COURT_LEVEL_LABELS[r.level] ?? r.level : "every court"} · {r.state_code ? NG_STATES[r.state_code] ?? r.state_code : "every state"} · from {r.effective_from}</p>
                </div>
                {r.citation && <p className="text-xs text-ink-muted">{r.citation}</p>}
                {r.note && <p className="text-xs text-ink-muted">{r.note}</p>}
                {r.provisions.length > 0 ? (
                  <ul className="mt-2 space-y-1">
                    {r.provisions.map((p) => (
                      <li key={p.id} className="flex flex-wrap items-baseline justify-between gap-2 text-sm text-ink">
                        <span>
                          ...
                          <span className="ml-1 font-mono text-xs text-ink-muted">{p.key}</span>
                        </span>
