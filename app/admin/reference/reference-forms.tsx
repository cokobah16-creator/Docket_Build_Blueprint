...
const ghostButton =
  "min-h-[44px] w-full rounded-lg border border-edge px-4 py-2.5 text-sm font-medium text-brand hover:bg-hover disabled:opacity-50 sm:w-auto";
const dangerButton =
    ...
    <Card>
      ...
      <CardBody className="space-y-4">
        ...
        {vacations.length === 0 ? (
          ...
          <>
            <p className="text-xs uppercase tracking-wide text-ink-muted">Recorded windows, newest first</p>
                ...
                <li key={v.id} className="rounded-lg border border-hairline p-4">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      {v.note && <p className="mt-1 text-sm text-ink-muted">{v.note}</p>}
                    </div>
      ...
      <CardBody className="space-y-4">
        ...
        <form method="get" action="/admin/reference" className="flex flex-wrap items-end gap-2">
          <div className="min-w-[12rem] flex-1">
            <label htmlFor="court-search" className="block text-sm font-medium text-ink">
              Find a court
            </label>
            <input
              id="court-search"
          ...
          <button type="submit" className="min-h-[44px] rounded-lg border border-edge bg-raised px-4 text-sm font-medium text-ink">
            Search
          </button>
          {search && (
        ...
        {courts.length === 0 ? (
          ...
          <>
            <p className="text-xs uppercase tracking-wide text-ink-muted">Most recently added</p>
            <ul className="divide-y divide-hairline">
              {courts.map((c) => (
