  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-sm text-ink">Pack key
          ...
          <span className="mt-1 block text-xs text-ink-muted">An existing key publishes the next version of that pack; a new key starts one.</span>
          {loadable && (
            <button type="button" onClick={() => loadLatest(key)}
                    className="mt-1 min-h-[36px] rounded-lg border border-edge bg-raised px-3 text-xs text-ink hover:border-brand">
              Load the latest version of {key} into this form
            </button>
          )}
        </label>
        <label className="text-sm text-ink">Name<input type="text" maxLength={120} value={name} onChange={(e) => setName(e.target.value)} className={field} /></label>
      </div>
      <fieldset>
        <legend className="text-sm text-ink">Matter types (none selected: every type)</legend>
      ...
      <label className="block text-sm text-ink">Definition (JSON)
        <textarea value={definition} onChange={(e) => setDefinition(e.target.value)} rows={18} className={`${field} font-mono text-base`} />
      </label>
      <label className="block text-sm text-ink">Note<input type="text" maxLength={1000} value={note} onChange={(e) => setNote(e.target.value)} placeholder="What changed in this version" className={field} /></label>
