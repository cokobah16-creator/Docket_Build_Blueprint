...
const fieldClass =
  "min-h-[44px] w-full rounded-lg border border-edge bg-raised px-3 py-2 text-sm text-ink";
const buttonClass =
...
export function FirmPlanControl({ firmId, plan }: { firmId: string; plan: string }) {
  ...
  return (
    <form action={action} className="space-y-2">
      ...
      <label className="block text-xs font-medium text-ink-muted" htmlFor={`plan-${firmId}`}>
        Plan
      </label>
...
export function FirmDomainControl({
  ...
  return (
    <form action={action} className="space-y-2">
      ...
      <label className="block text-xs font-medium text-ink-muted" htmlFor={`domain-${firmId}`}>
        Custom domain
      </label>
      <input
        id={`domain-${firmId}`}
  ...
  return (
    <div className="space-y-4">
      <form action={startAction} className="space-y-2">
        ...
        <p className="text-xs font-medium text-ink-muted">Step 1 — ask Vercel to serve it</p>
        <p className="text-xs text-ink-muted">
      ...
      <form action={liveAction} className="space-y-2">
        <input type="hidden" name="requestId" value={requestId} />
        <p className="text-xs font-medium text-ink-muted">Step 2 — map it and go live</p>
        <p className="text-xs text-ink-muted">
      ...
      <form action={rejectAction} className="space-y-2">
        <input type="hidden" name="requestId" value={requestId} />
        <p className="text-xs font-medium text-ink-muted">Or turn it down</p>
        <input
          name="note"
          maxLength={500}
          required
