  ...
  return (
    <div className="space-y-6">
      ...
      <section id="documents" className="space-y-4">
        <h2 className="font-heading text-lg font-semibold text-ink">Are the documents actually there?</h2>
        <p className="text-sm text-ink-muted">
              ...
              <CardBody>
                <dl className="grid gap-3 sm:grid-cols-3">
                  {[
                    ...
                    <div key={label}>
                      <dt className="text-xs uppercase tracking-[0.06em] text-ink-muted">{label}</dt>
      ...
      <section id="settlement" className="space-y-4">
        <h2 className="font-heading text-lg font-semibold text-ink">Settlement</h2>
      ...
      <section id="queue" className="space-y-4">
        <h2 className="font-heading text-lg font-semibold text-ink">The notification queue</h2>
                        ...
                        <TR key={`${row.status}-${row.channel}-${row.event}`}>
                          ...
                          <TD className="tabular-nums">
                            ...
                            {Number(row.most_send_attempts ?? 0) > 1 && (
                              <span className="block text-xs text-ink-muted">{row.most_send_attempts} sends</span>
                            )}
                          </TD>
                    ...
                    <div className="space-y-2 border-t border-hairline px-5 py-4">
                      ...
                      <ul className="space-y-1">
                        {group.rows
                          ...
                          .map((r) => (
                            <li key={`err-${r.status}-${r.channel}-${r.event}`} className="text-sm text-ink">
      ...
      <section id="cost" className="space-y-4">
        <h2 className="font-heading text-lg font-semibold text-ink">What messages cost</h2>
        <p className="text-sm text-ink-muted">
        ...
        {costRead.error ? (
          ...
          <ul className="space-y-4">
            {costGroups.map((g) => (
              <li key={g.key}>
                <Card>
                  <CardHeader title={g.label} action={g.matters !== null ? <span className="text-xs text-ink-muted">{g.matters} open matter{g.matters === 1 ? "" : "s"}</span> : undefined} />
                  <Table>
                    <THead>
        ...
        <Card>
          ...
          <CardBody className="space-y-4">
            ...
            {ratesRead.error ? (
              ...
              <p className="text-sm text-ink-muted">No rate has been entered. Until one is, every message is unpriced.</p>
            ) : (
              <Table>
                ...
                <TBody>
                  {ratesRead.rows.map((r) => (
                    <TR key={`${r.provider}-${r.channel}-${r.effective_from}`}>
                      ...
                      <TD className="break-words text-xs text-ink-muted">{r.note ?? ""}</TD>
                    </TR>
                  ))}
                </TBody>
              </Table>
      ...
      <section id="webhooks" className="space-y-4">
        <h2 className="font-heading text-lg font-semibold text-ink">What providers sent us</h2>
            ...
            <dl className="space-y-2">
              {Object.entries(OUTCOME_MEANING).map(([outcome, meaning]) => (
                <div key={outcome} className="sm:flex sm:gap-3">
                  <dt className="text-sm font-medium text-ink sm:w-28 sm:shrink-0">{outcome}</dt>
                  <dd className="text-sm text-ink">{meaning}</dd>
                </div>
              ))}
            </dl>
          </CardBody>
