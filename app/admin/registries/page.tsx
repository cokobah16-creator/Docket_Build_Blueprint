...
export default async function RegistriesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-heading text-26 font-semibold text-docket-hunter">Court registries</h1>
      ...
      {registries.length === 0 ? (
        ...
        registries.map((r) => {
          ...
          return (
            <Card key={r.id}>
              <CardHeader title={r.name} action={<span className={r.status === "active" ? "text-13 font-medium text-emerald-800" : "text-13 font-medium text-red-800"}>{r.status}</span>} />
              <CardBody className="space-y-3">
                ...
                <div>
                  <p className="text-13 font-semibold uppercase tracking-wide text-ink-muted">Who acts for it</p>
                  {its.length === 0 ? (
                    <p className="mt-1 text-15 text-amber-800">Nobody yet — it cannot publish until a registrar is added.</p>
                  ) : (
                    <ul className="mt-1 divide-y divide-hairline">
                      {its.map((m) => {
                        const p = personById.get(m.user_id);
                        return (
                          <li key={m.user_id} className="flex flex-wrap items-center justify-between gap-2 py-1.5 text-15">
...
function PilotCounts({ row, timezone }: { row: RegistryPilotHealthRow | undefined; timezone: string }) {
  ...
  return (
    <div>
      <p className="text-13 font-semibold uppercase tracking-wide text-ink-muted">The pilot, measured</p>
      <dl className="mt-1 grid grid-cols-2 gap-x-4 gap-y-1 sm:grid-cols-3">
        {cells.map(([label, n]) => (
          <div key={label} className="flex items-baseline justify-between gap-2 text-15">
