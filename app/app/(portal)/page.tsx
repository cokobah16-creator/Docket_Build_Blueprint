...
export default async function ClientDashboard() {
  ...
  const firmLine = firm && (
    <>
      ...
      <span className="mt-0.5 block text-[13px] text-ink-muted">{firm.name}</span>
    </>
  );
  ...
  return (
    <Screen className="gap-4">
      ...
      <nav aria-label="Quick actions" className="grid grid-cols-5 gap-[7px] md:gap-3">
        {quickActions.map((a) => (
          <Link
            key={a.label}
            href={a.href}
            className="flex min-h-16 flex-col items-center justify-center gap-1.5 rounded-[9px] border border-hairline bg-raised px-0.5 py-2.5 text-center text-[11px] font-semibold leading-tight text-brand hover:bg-hover md:min-h-[72px] md:rounded-xl md:text-[13px]"
          >
                    ...
                    <li key={d.id}>
                      <Link href={d.matter_id ? `/app/matters/${d.matter_id}?tab=documents` : `/app/appointments/${d.appointment_id}`} className="flex items-center justify-between gap-3 border-t border-hairline px-[17px] py-3 hover:bg-sunken">
                        <span className="flex min-w-0 items-center gap-2.5">
                          ...
                          <span className="truncate text-[13.5px] text-ink">{d.name}</span>
                        </span>
                        <span className="shrink-0 text-[11.5px] text-ink-muted">{new Intl.DateTimeFormat("en-GB", { dateStyle: "medium", timeZone: tz }).format(new Date(d.created_at))}</span>
                      </Link>
                    </li>
                  ))}
                </ul>
        ...
        <Card>
          ...
          {nextAppointment ? (
            <CardBody className="space-y-2.5">
              <p className="text-[14.5px] font-semibold leading-snug text-ink md:text-base">{new Intl.DateTimeFormat("en-GB", { dateStyle: "full", timeStyle: "short", timeZone: tz }).format(new Date(nextAppointment.starts_at))}</p>
              <p className="text-[13px] text-ink-muted"><span className="font-mono">{nextAppointment.reference}</span> · {nextAppointment.mode.replace("_", " ")}</p>
              ...
              {matters.map((m) => (
                <li key={m.id}>
                  <Link href={`/app/matters/${m.id}`} className="block border-t border-hairline px-[17px] py-[13px] hover:bg-sunken">
                    ...
                    <p className="mt-1 text-xs text-ink-muted"><span className="font-mono">{m.reference}</span>{m.lawyer_names.length ? ` · ${m.lawyer_names[0]}` : ""}</p>
                    {m.last_update && <p className="mt-0.5 truncate text-xs text-ink-muted">{m.last_update.title} · {fmt.format(new Date(m.last_update.occurred_at))}</p>}
      ...
      <p className="pt-0.5 text-center text-[11.5px] text-ink-muted">
