...
export default async function AppointmentPage({
  ...
  const appointmentResult = await supabase
    ...
    .eq("id", id)
    .maybeSingle();
  if (appointmentResult.error) throw new Error(`Consultation could not be loaded: ${appointmentResult.error.message}`);
  const appt = (appointmentResult.data ?? null) as Appt | null;
  if (!appt) notFound();

  const [serviceResult, lawyerResult, invoiceResult, notesResult, firm, readinessResult, documentResult, requestResult] = await Promise.all([
    appt.service_id
      ? supabase.from("services").select("name, duration_min").eq("id", appt.service_id).maybeSingle()
      : Promise.resolve({ data: null, error: null }),
    appt.lawyer_id
      ? supabase.from("lawyer_public").select("full_name, title").eq("id", appt.lawyer_id).maybeSingle()
      : Promise.resolve({ data: null, error: null }),
    appt.invoice_id
      ? supabase.from("invoices").select("id, number, status, total_minor, currency").eq("id", appt.invoice_id).maybeSingle()
      : Promise.resolve({ data: null, error: null }),
  ...
  if (serviceResult.error) throw new Error(`Consultation service could not be loaded: ${serviceResult.error.message}`);
  if (lawyerResult.error) throw new Error(`Consultation lawyer could not be loaded: ${lawyerResult.error.message}`);
  if (invoiceResult.error) throw new Error(`Consultation invoice could not be loaded: ${invoiceResult.error.message}`);
  if (notesResult.error) throw new Error(`Consultation notes could not be loaded: ${notesResult.error.message}`);
  if (readinessResult.error) throw new Error(`Consultation readiness could not be loaded: ${readinessResult.error.message}`);
  if (documentResult.error) throw new Error(`Consultation documents could not be loaded: ${documentResult.error.message}`);
  if (requestResult.error) throw new Error(`Document requests could not be loaded: ${requestResult.error.message}`);

  const readiness = (readinessResult.data ?? null) as AppointmentReadiness | null;
  const docs = (documentResult.data ?? []) as DocumentRow[];
  const requests = (requestResult.data ?? []) as DocumentRequestRow[];
  const versionResult = docs.length
  ...
  if (versionResult.error) throw new Error(`Document versions could not be loaded: ${versionResult.error.message}`);
  ...
  const formResult = intakeItem?.ref
    ? await supabase.from("intake_forms").select("id, firm_id, service_id, name, schema").eq("id", intakeItem.ref).maybeSingle()
    : { data: null, error: null };
  if (formResult.error) throw new Error(`Intake form could not be loaded: ${formResult.error.message}`);
  const questions = (((formResult.data ?? null) as IntakeForm | null)?.schema?.questions ?? []);
  const notes = notesResult.data as { client_summary: string | null; advice_given: string | null; follow_up: string | null; updated_at: string } | null;
  const svc = serviceResult.data as { name: string; duration_min: number } | null;
  const law = lawyerResult.data as { full_name: string | null; title: string | null } | null;
  const inv = invoiceResult.data as { id: string; number: string; status: string; total_minor: number; currency: string } | null;
  ...
  const ownerFirmResult = await supabase.from("firm_public").select("id, name").eq("id", appt.firm_id).maybeSingle();
  if (ownerFirmResult.error) throw new Error(`Consultation firm could not be loaded: ${ownerFirmResult.error.message}`);
  const apptFirm = (ownerFirmResult.data as { id: string; name: string } | null) ?? null;
