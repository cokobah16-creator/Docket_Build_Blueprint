    ...
    <div className="space-y-3">
          ...
          {readiness.items.map((i) => (
            <li key={i.kind} className="flex items-start gap-3 py-2">
              ...
              <div className="min-w-0">
                <p className={i.satisfied ? "text-[13.5px] text-ink-muted" : "text-[13.5px] font-medium text-ink"}>{i.label}</p>
                <p className="text-[12px] text-ink-muted">{i.detail}</p>
                ...
                {!i.satisfied && i.kind === "payment" && <p className="text-[12px] text-ink-muted">Pay in the panel above.</p>}
                {!i.satisfied && i.kind === "documents" && <p className="text-[12px] text-ink-muted">Upload each one in Documents below — the request is listed there.</p>}
              </div>
            </li>
          ))}
        </ul>
      ...
      {toAsk.length > 0 && (
        <form className="space-y-3 rounded-lg border border-hairline p-3" onSubmit={(e) => { e.preventDefault(); submit(); }}>
          <p className="text-[13.5px] font-medium text-ink">Still to answer</p>
          {error && <Alert kind="error">{error}</Alert>}
          {toAsk.map((q) => (
            <div key={q.key}>
              <label htmlFor={`q-${q.key}`} className="text-sm font-medium text-ink">{q.label}</label>
              {q.help && <p className="text-xs text-ink-muted">{q.help}</p>}
              {q.type === "longtext" ? (
                ...
                <div className="mt-1 space-y-1">
                  {(q.options ?? []).map((o) => {
                    ...
                    return (
                      <label key={o} className="flex min-h-[44px] items-center gap-2 text-sm text-ink">
