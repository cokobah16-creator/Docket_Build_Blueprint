    ...
    <>
      ...
      <Screen>
        <header>
          ...
          <p className="mt-1 text-[13px] text-ink-muted">{firm?.name ?? "Your firm"}{lawyers.length ? ` · ${lawyers.map((l) => l.full_name ?? l.title).join(", ")}` : ""}</p>
        </header>
          ...
          <nav aria-label="Matter sections" className="-mx-4 flex gap-2 overflow-x-auto px-4 md:-mx-6 md:px-6 lg:-mx-8 lg:px-8 xl:mx-0 xl:px-0">
            {TABS.map(([key, label]) => (
              <Link key={key} href={`/app/matters/${matter.id}?tab=${key}`} aria-current={tab === key ? "page" : undefined}
                className={cn("flex min-h-11 shrink-0 items-center rounded-full border px-3.5 text-[12.5px] font-medium", tab === key ? "border-brand bg-brand text-brand-on" : "border-edge bg-raised text-ink")}>
                {label}
              </Link>
            ))}
          </nav>
...
async function InvoicesSection({ supabase, matterId, tz }: { supabase: SB; matterId: string; tz: string }) {
  ...
  if (invoices.length === 0) return <p className="px-5 py-8 text-center text-sm text-ink-muted">No invoices on this matter.</p>;
      ...
      {invoices.map((inv) => {
        ...
        return (
          <li key={inv.id} className="flex items-center justify-between gap-3 px-5 py-4">
            <div>
              <Link href={`/app/payments/${inv.id}`} className="text-sm font-medium text-ink underline">{inv.number}</Link>
