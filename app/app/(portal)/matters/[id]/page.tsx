      ...
      {invoices.map((inv) => {
        ...
        return (
          <li key={inv.id} className="flex items-center justify-between gap-3 px-5 py-4">
            <div>
              <Link href={`/app/payments/${inv.id}`} className="text-15 font-medium text-ink underline">{inv.number}</Link>
            ...
            <div className="flex items-center gap-2">
              ...
              {payable && <form action={pay}><button type="submit" className="rounded-lg bg-brand px-3 py-1.5 text-13 font-medium text-brand-on hover:opacity-90">Pay {formatMoneyMinor(outstanding, inv.currency)}</button></form>}
            </div>
          </li>
        );
      })}
