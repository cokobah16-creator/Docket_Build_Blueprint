...
**Validated:** all 54 migrations, the seed and 32 SQL suites (1,457 checks) run clean on PostgreSQL 16. The onboarding runbook is `docs/ONBOARDING_A_FIRM.md`; every server function is catalogued in `docs/RPC_REFERENCE.md`.
...
| `record_consent(firm)` | any signed-in person, for themselves | records acceptance of the firm's terms and privacy notice at the versions it has published; refuses a firm that is not active or has not published them; a repeat call writes nothing |
