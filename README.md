...
**Validated:** all 54 migrations, the seed and 32 SQL suites (1,466 checks) run clean on PostgreSQL 16. The onboarding runbook is `docs/ONBOARDING_A_FIRM.md`; every server function is catalogued in `docs/RPC_REFERENCE.md`.
...
5. Point Paystack's webhook at `/functions/v1/paystack-webhook` (Paystack dashboard → Settings → API Keys & Webhooks; there is no API for it).
   `scripts/configure-providers.sh` sets the Auth Site URL, redirect allow-list, Docket email
   template, custom SMTP, Google OAuth and phone OTP when their provider variables are present. It
   needs a Supabase personal access token (`SUPABASE_ACCESS_TOKEN`), `SUPABASE_PROJECT_REF` and
   `APP_URL`; see the script header for the provider variables. Afterward run `npm run auth:check`
   with those three values to fail fast on configuration drift.
...
Each suite ends with `NOTICE:  ALL CHECKS PASSED` (1,466 `PASS` lines across 32 suites; `scripts/check-suite-counts.sh` holds each suite to its own floor, so these are counts CI keeps true rather than counts somebody remembered to update). Coverage: client isolation (matters, updates, documents, invoices), staff isolation across firms, MFA gating of staff writes, the anonymous surface, slot computation with breaks, booking and double-booking, the 15-minute hold, the payment cascade and duplicate-webhook idempotency, the court-update form, consultation notes (internal notes invisible to clients), audit-log access and immutability, the hold-release job; self-serve firm creation and its defaults, slug validation, the three-firm cap, platform admins seeing lifecycle rows and no content; the court directory (platform-wide vs firm-private), holidays and vacations, practitioner fields; service of process across firms (record + served document visible to the served firm, nothing else; acknowledgement once; clients see progress); manual invoicing and the chase list; and the admin surfaces — domain requests, role changes and member removal, policy and template validation, what a platform admin can and cannot see on the health screens, and the rate limiter; matter walls (a restricted matter invisible to a colleague outside its team through rows, bytes, threads, the overview and every RPC); document reads as the door to the bytes; document requests (asked once, answered once, withdrawn never deleted, walled like the documents); and conflict checks (the firm's own register only, a match behind a wall reported without the matter, the lawyer's decision recorded once, and — opt-in — no client joined to a matter until it is cleared); and the onboarding checklist and import (one readiness answer in the booking engine's order, a skipped step stored with who and why, an import staged and filed a bounded batch at a time with dates and old file numbers kept, unknown statuses refused, every client invited and never linked by a number in a file, a phone nobody can read said rather than guessed, and a batch short of its rows refused whole); and the pre-consultation check-in (readiness computed from real rows, a held booking the database refuses to confirm until what the firm asked for is in, the client answering only what is missing, and a conflict item the client sees only as "the firm's own checks"); and retries (a court update posted twice with the same reference is one posting, a message sent twice with the same id lands once, and an upload that stopped leaves a row that answers no request, is counted by the platform, and can be retired by whoever made it); and notifications (the same message to the same person is one row a day, the dispatcher claims before it sends and two claims never overlap, accepted is written from the provider's answer and delivered only from its verified receipt, a transient failure comes back with a growing delay and a permanent one does not, the API can change nothing but read_at whatever column is added, and a rate is what the platform entered — an unpriced message is unpriced); and the legal diary (a court date records who made it and the notice it came from, and is evidenced or merely claimed; a deadline is counted by the database from the rules the platform entered — calendar, clear or working days, months, time stopped in vacation, rolled past a day the court does not sit — with every skipped and rolled day and the reference data it relied on written into the row, proposed until a lawyer confirms it, superseded rather than edited, discharged with a note, never a client's to see); and workflow packs (a versioned, immutable catalogue the platform publishes and a firm installs, which adds the stages the firm lacks and recognises the ones it has without rewriting a label or moving a matter; a stage change as one act that tells the client, closes or reopens the file, and starts the stage's tasks once); and templates and execution (a template that names a fact Docket cannot fill is refused when it is saved, a generation that finds the matter has no value for one refuses and says which rather than guessing, a signature is refused from somebody who has not opened those exact bytes or who types a name that is not their own, it records the version and its checksum, it locks the document so no further version can be added, and an instrument the law executes on paper is recorded — witness, attestation, stamp and registration — rather than signed here); and the pilot baseline (every figure computed from the firm's own rows over a named window, a consultation never written up counted as unrecorded rather than attended or missed, an inference labelled as one, money per currency, the record append-only and unreadable by a client, and what the firm says about the years before Docket kept as a stated claim beside the figures and never inside them); and search (a colleague outside a walled matter's team cannot find it by any word in it, by its suit number or by its Docket reference; a client never finds an internal update, a staff task, the adverse-party register or an internal note; nothing crosses a firm; and the search function is asserted not to be SECURITY DEFINER, because that one word would turn it into a wall-free read of everything); and verified delegation (an authority to act for a client is bound only by redeeming a single-use token AS the person the firm named — a link alone binds nobody and a phone number alone binds nobody; it reaches documents or money only where the firm said so, never a signature; it stops granting the moment it expires with no job having run; and the firm or the client can end it, with the record kept either way); and firm-to-firm collaboration (a referral, joint retainer or agency is worth nothing until the other firm accepts; what they read is a snapshot through one definer view and never the matter row; documents are handed over one named version at a time, so a later version is not shared by an earlier row; an internal note is never shared however wide the arrangement; and ending it — by either firm, or by its own last day with no job run — closes every door at once, though nothing recalls a copy already downloaded); and the partner API (a key is hashed and shown once, carries only the scopes it was issued with, speaks for one firm and no other, and opens nothing once revoked, expired or its firm suspended — every one of those decided by a database function the HTTP layer cannot reach around, since it holds the key and never a firm id; and an event feed a partner pages by sequence, so nothing is missed and nothing is processed twice); and the calendar feed (a subscribed URL carries one person's own diary and nobody else's, the matter wall is applied to its owner by name because a calendar app fetches without a session, a member who has left the firm and a firm that has been suspended both stop having a diary, issuing a second URL revokes the first the same second, and a client's name is in it only where the person deliberately said so); and the words inside a document (a colleague outside a walled matter's team finds a phrase in its file before the wall goes up and not after, a client finds the words in what their firm shared with them and not one word of the internal advice on the same matter, only the current version is searched, nothing crosses a firm, and the two functions that write the text are executable by neither anon nor authenticated with the table beside them shut); and the court-registry pilot (a registry is a principal that is not a firm and reads zero rows of any firm's matters, court events, decisions or notifications, even after firms have acted on its notices; a firm sees the court's notices only for suit numbers it holds at that court, never a draft, never another firm; a colleague off a walled matter's team sees its notice before the wall and not after; nothing reaches a diary until a lawyer confirms, and confirming attaches to a same-day sitting rather than duplicating it; a registry-sourced date a lawyer moves by hand stops being the registry's; a withdrawal is stamped on the sitting and vacates nothing; and the platform measures the pilot in counts that name no firm; a listing is confirmed only by an owner, admin or lawyer, a value longer than its column is refused rather than trimmed, and a colleague who has left the firm is told nothing — the root of which, remove_member() leaving matter_lawyers rows behind, is fixed here for all four fan-outs that read them); and booking consent (a client's acceptance of a firm's terms and privacy notice is recorded for the caller alone, only at the versions the caller was shown and only when those are the versions the firm has published — a stale version is refused and nothing is written — once however often it is asked, and never for a firm that is not active or whose terms or privacy notice is missing or still a draft).
...
| `record_consent(firm, terms_version, privacy_version)` | any signed-in person, for themselves | records acceptance of the firm's terms and privacy notice at the versions shown, only when they are the versions the firm has published (a stale version is refused, DKC01, and nothing is written); refuses a firm that is not active or has not published them; a repeat call writes nothing |
